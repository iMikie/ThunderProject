This is the main stack script, basically library scripts for the entire app.  Thus, here is my JSON library and libFormURL librariews.

local savedProperties

//Take a list of fields (on the current card) and clear each field
command clearFields fieldList
   repeat for each item fieldName in fieldList do
      put "" into field fieldName
   end repeat
end clearFields

command clearMessageField
      put "" into field "message"
end clearMessageField

on mouseDown
   if the commandKey is down  and the shiftKey is down then
      set the properties of the target to savedProperties
      put jsonFromArray(savedProperties) into field 1
   else if the commandKey is down  then
      put the properties of the target into savedProperties
      delete variable savedProperties["rect"]
      delete variable savedProperties["name"]
      delete variable savedProperties["contents"]
      put the width of the target into savedProperties["width"]
      put the height of the target into savedProperties["height"]
   end if
end mouseDown
   //
   //----------------------------------------------------------
